...
- `.agents/context/blog-style-guide.yaml` 확인 (톤, 페르소나, 플랫폼별 규칙)
- `.agents/workflows/translation.yaml` 확인 (5단계 번역용)
- `.env`에 `GEMINI_API_KEY` 설정 확인 (번역 스크립트에 필요)
- 페르소나 SoT: `docs-nextain/05. 디자인/naia-persona.md` (캐치프레이즈, 성격, 말투)
...
### 8단계: 배포 & 크로스포스팅
관리자 에디터 페이지 사용: `http://localhost:3000/{lang}/admin/blog/editor` (개발 서버 + 관리자 세션 필요)

**배포:**
1. 드롭다운에서 포스트 선택
2. Split/Preview 모드로 내용 확인
3. 커밋 메시지 입력 (기본값: `blog: update {slug}`)
4. **Deploy** 클릭 — git commit + push 실행

**크로스포스팅 (영어 버전 기반):**
1. **Dev.to**: Dev.to 버튼 클릭 → `canonical_url`을 naia.nextain.io로 설정하여 자동 게시
2. **Reddit**: 서브레딧명 입력 → Reddit 버튼 클릭 → 브라우저에서 Reddit 게시 페이지 열림 → 제목/본문 수정 후 직접 게시
...
- [ ] OG 태그 확인
- [ ] sitemap에 포함 확인
- [ ] 배포 완료 (git commit + push)
- [ ] Dev.to 크로스포스팅 (canonical_url 포함)
- [ ] Reddit 크로스포스팅 (submit URL 열림, 사용자가 게시)
```
...
| `src/app/[lang]/(protected)/admin/blog/editor/page.tsx` | 관리자 에디터 (편집/미리보기/배포/크로스포스팅) |
| `src/lib/crosspost/devto.ts` | Dev.to API 연동 |
| `src/lib/crosspost/reddit.ts` | Reddit submit URL 생성 |
| `src/app/api/admin/deploy/route.ts` | Git 배포 API (개발 환경 전용) |
| `src/app/api/admin/crosspost/route.ts` | 크로스포스팅 API |
