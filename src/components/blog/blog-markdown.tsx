    ...
    <>
      <article className="mt-8 space-y-4 text-base leading-8 text-muted-foreground">
        <ReactMarkdown
          ...
          components={{
            h1: ({ children }) => (
              <h1 className="mb-4 mt-10 text-2xl font-bold text-foreground">{children}</h1>
            ),
            h2: ({ children }) => (
              <h2 className="mb-2 mt-8 clear-both text-xl font-semibold text-foreground">{children}</h2>
            ),
            h3: ({ children }) => (
              <h3 className="mb-1 mt-6 clear-both text-lg font-semibold text-foreground">{children}</h3>
            ),
            h4: ({ children }) => (
            ...
            img: ({ src, alt }) => {
              ...
              const resolvedSrc =
                srcStr.startsWith("/") || srcStr.startsWith("http")
                  ? srcStr
                  : `/posts/${slug}/${srcStr}`;
              ...
              return (
                <img
                  src={resolvedSrc}
                  alt={alt ?? ""}
                  className="my-4 h-auto max-w-full cursor-pointer rounded-lg border border-border/30 shadow-sm transition-opacity hover:opacity-80"
                  loading="lazy"
            ...
            p: ({ node, children }) => {
              ...
              if (hasImg) {
                ...
                const rawAlt = imgHast && "properties" in imgHast
                  ? String((imgHast.properties as Record<string, unknown>)?.alt ?? "")
                  : "";
                const isFloat = rawAlt.includes("#float");
                const alt = rawAlt.replace(/#float/g, "").trim();
