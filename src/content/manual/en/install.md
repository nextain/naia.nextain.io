Cafelua OS is designed primarily for Linux environments, focusing on Immutable OSs like Bazzite or secure sandbox environments like Flatpak.
...
Since Cafelua OS features an AI that "controls the OS directly," the deployment method is an important consideration.
...
For advanced users, Cafelua OS provides a BlueBuild recipe to bake the app directly into a Bazzite image. This deeply integrates the AI into the OS, allowing the AI avatar to greet you immediately upon boot.
...
3. **Run the Packaging Build**
   Execute this in the project root directory (cafelua-os).
   ```bash
