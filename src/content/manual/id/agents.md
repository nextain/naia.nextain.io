Tab ini mengelola semua agen (termasuk Naia utama) dan sub-agen yang berjalan dalam Naia.

![Agents Tab](agents-tab.png)

## Daftar Agen
- Anda dapat melihat daftar semua kartu agen yang sedang aktif.
- Ketika tugas yang kompleks dimulai, Naia dapat memunculkan sub-agen latar belakang untuk memproses tugas secara paralel.

## Manajemen Berkas
Mengeklik tombol **Berkas** di bagian bawah kartu agen menampilkan daftar berkas yang dihasilkan dalam ruang kerja lokal agen tersebut.

- **Baca Berkas**: Klik berkas dari daftar untuk melihat isinya di area teks.
- **Edit Berkas**: Anda dapat mengedit konten di dalam area teks dan klik **Simpan** untuk segera memperbarui berkas.

## Manajemen Sesi
Anda juga dapat mengelola sesi percakapan aktif untuk agen-agen ini.
- **Hapus**: Hapus sesi yang tidak perlu.
- **Padatkan**: Meringkas riwayat sesi yang sangat panjang untuk membebaskan memori sistem.