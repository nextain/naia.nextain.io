import type { Dictionary } from "./types";

const hi: Dictionary = {
  "locale": "hi",
  "meta": {
    "title": "Naia — The Next Generation AI OS",
    "description": "आपका AI यहां रहता है। ओपन सोर्स, क्लाउड से लोकल AI तक — आप चुनते हैं। VRM 3D अवतार के साथ वॉइस चैट, 70+ स्किल, OpenClaw AI गेटवे, मेमोरी और व्यक्तित्व आपकी मशीन पर। Steam-तैयार सुरक्षित Linux जहां आपका AI रहता है।",
  },
  "common": {
    "loading": "लोड हो रहा है...",
    "loadingShort": "लोड हो रहा है...",
    "error": "एक त्रुटि हुई",
    "save": "सहेजें",
    "cancel": "रद्द करें",
    "delete": "हटाएँ",
    "confirm": "पुष्टि करें",
    "copy": "प्रतिलिपि",
    "copied": "नकल की गई",
    "comingSoon": "जल्द आ रहा है",
    "backTo": "वापस",
    "prev": "पिछला",
    "next": "अगला",
    "page": "पेज"
  },
  "header": {
    "home": "घर",
    "pricing": "मूल्य निर्धारण",
    "faq": "अक्सर पूछे जाने वाले प्रश्न",
    "download": "डाउनलोड करें",
    "login": "लॉग इन करें",
    "dashboard": "डैशबोर्ड",
    "toggleMenu": "मेनू टॉगल करें",
    "manual": "मैनुअल",
    "blog": "ब्लॉग",
    "contribute": "योगदान",
  },
  "footer": {
    "brand": "नाया",
    "tagline": "जहां प्रौद्योगिकी भावना से मिलती है",
    "links": {
      "privacy": "गोपनीयता नीति",
      "terms": "सेवा की शर्तें",
      "refund": "धनवापसी नीति",
      "contact": "संपर्क करें",
      "about": "Nextain के बारे में",
      "donation": "दान करें"
    },
    "copyright": "© 2026 nextain Inc. All rights reserved.",
    "companyInfo": "nextain Inc. | CEO Byeongseok Yang | Business Registration No. 529-81-03749\n77, Maesong-Gosaek-ro 422beon-gil, Maesong-myeon, Hyohaeng-gu, Hwaseong-si, Gyeonggi-do, South Korea, Bldg B #107 | support@nextain.io"
  },
  "auth": {
    "loginTitle": "लॉगइन/साइन अप करें",
    "loginDescription": "अपने सामाजिक खाते से शुरुआत करें",
    "googleLogin": "Google के साथ जारी रखें",
    "discordLogin": "Discord के साथ जारी रखें",
    "logout": "लॉगआउट करें",
    "callbackRedirecting": "डेस्कटॉप ऐप पर रीडायरेक्ट किया जा रहा है...",
    "callbackManualPrefix": "यदि यह स्वचालित रूप से नहीं खुलता है,",
    "callbackManualLink": "यहां क्लिक करें",
    "callbackManualSuffix": "."
  },
  "home": {
    "hero": {
      "title": "आपका AI यहां रहता है। आपका OS, आपके नियम।",
      "subtitle": "एक ओपन-सोर्स AI OS जो चुनाव और नियंत्रण आपके हाथों में लौटाता है। अपना AI चुनें (लोकल मॉडल सहित), इसकी मेमोरी और व्यक्तित्व को आकार दें। एक सुरक्षित, Steam-तैयार Linux पर रहने वाला आपका अपना AI।",
      "cta": "निःशुल्क आरंभ करें",
      "secondaryCta": "डाउनलोड करें"
    },
    "features": {
      "title": "आपका AI, आपके नियम",
      "subtitle": "अपने AI पर पूर्ण संप्रभुता — चयन, नियंत्रण और स्वामित्व वापस आपके हाथों में",
      "items": {
        "companion": {
          "title": "आपका अपना AI अवतार",
          "description": "VRM 3D अवतार के साथ अपना AI कैरेक्टर बनाएं। रूप, भाव और भावनाएं कस्टमाइज़ करें। भविष्य: रीयल-टाइम वीडियो अवतार, गाना, साथ गेमिंग।"
        },
        "multiProvider": {
          "title": "AI चुनने की स्वतंत्रता",
          "description": "7 क्लाउड प्रदाता (Gemini, Claude, GPT, Grok, zAI) से लोकल AI (Ollama) तक। आपका OS आपका AI तय नहीं करता। आप चुनते हैं।"
        },
        "voice": {
          "title": "आपके AI की आवाज़",
          "description": "5 TTS इंजन के साथ अपने AI को मनचाही आवाज़ दें। स्वाभाविक रूप से बात करें और लिप-सिंक के साथ इसे जीवंत होते देखें।"
        },
        "tools": {
          "title": "पूरा OS एक उपकरण के रूप में",
          "description": "फ़ाइलें, टर्मिनल, वेब, ब्राउज़र — आपका AI OS को ही अपने उपकरण के रूप में उपयोग करता है। कहें 'एक फ़ाइल बनाओ' और यह वास्तव में बना देता है।"
        },
        "desktop": {
          "title": "संपूर्ण लोकल नियंत्रण",
          "description": "मेमोरी, व्यक्तित्व, सेटिंग्स — सब आपकी मशीन पर संग्रहीत। सच में निजी AI, क्लाउड निर्भरता से मुक्त। बूटेबल USB OS इमेज उपलब्ध।"
        },
        "skills": {
          "title": "70+ स्किल, असीमित रूप से विस्तार योग्य",
          "description": "मौसम, GitHub, Slack, Spotify, Discord — 70 बिल्ट-इन स्किल + 5,700+ ClawHub कम्युनिटी स्किल। जो चाहिए वो जोड़ें या अपनी बनाएं।"
        },
        "discord": {
          "title": "हमेशा आपके साथ",
          "description": "जब आप दूर हों तब भी Discord DM से अपने AI से बात करें। आपका AI हमेशा आपके साथ है।"
        },
        "daemon": {
          "title": "हमेशा चालू AI",
          "description": "OpenClaw गेटवे 24/7 बैकग्राउंड में चलता है। ऐप बंद करने पर भी आपका AI जीवित रहता है — संदेश प्राप्त करता और काम करता रहता है।"
        },
        "memory": {
          "title": "बढ़ती हुई स्मृति",
          "description": "Naia बातचीत के दौरान स्वाभाविक रूप से आपका नाम, प्राथमिकताएँ, जन्मदिन याद रखती है। यादें डेस्कटॉप, Discord DM सहित सभी चैनलों में बनी रहती हैं और केवल आपके स्थानीय डिवाइस पर संग्रहीत होती हैं।"
        },
        "openSource": {
          "title": "AI द्वारा संरक्षित ओपन सोर्स",
          "description": "दोहरा लाइसेंस: सोर्स कोड Apache 2.0, AI संदर्भ CC-BY-SA 4.0। AI एजेंट स्वयं स्रोत का श्रेय देते हैं, लाइसेंस का पालन करते हैं, और ओपन सोर्स इकोसिस्टम को बनाए रखते हैं — एक नया योगदान ढांचा।"
        },
        "vision": {
          "title": "दृष्टि: अपरिवर्तनीय गेमिंग OS + AI",
          "description": "लक्ष्य: Bazzite पर निर्मित, एक अपरिवर्तनीय OS जो कभी नहीं टूटता। Steam/Proton गेमिंग पूरी तरह तैयार होगी। इस सुरक्षित, शक्तिशाली गेमिंग Linux पर आपका अपना AI रहेगा — अपने AI के साथ गाएं, खेलें और रचना करें, Physical AI तक।"
        }
      }
    },
    "usbBoot": {
      "title": "USB लगाएँ, AI तुरंत चलाएँ",
      "description":
        "कोई इंस्टॉलेशन नहीं, कोई कॉन्फ़िगरेशन नहीं। बस Naia OS USB को अपने लैपटॉप में लगाएँ और चालू करें — आपकी अपनी AI तुरंत जीवंत हो जाएगी। इसे आज़माएँ, और पसंद आए तो हार्ड ड्राइव पर इंस्टॉल करें।",
      "imageAlt": "Naia OS चलाने के लिए लैपटॉप में USB लगाना"
    },
    "comparison": {
      "title": "Naia क्यों",
      "subtitle": "अन्य AI उपकरण बंद 'उपकरण' हैं। Naia 'आपका अपना AI' है जो ओपन सोर्स है — गेमिंग OS पर AI की एक दृष्टि।",
      "headers": { "category": "श्रेणी", "others": "अन्य AI उपकरण", "naia": "Naia" },
      "rows": {
        "ui": { "category": "दर्शन", "others": "AI को उपकरण के रूप में उपयोग करें", "naia": "AI को OS दें। साथ रहें।" },
        "target": { "category": "लक्ष्य", "others": "केवल डेवलपर्स", "naia": "हर कोई जो अपना AI चाहता है" },
        "llm": { "category": "AI चयन", "others": "प्लेटफ़ॉर्म तय करता है", "naia": "7 क्लाउड + लोकल AI — आप तय करें" },
        "avatar": { "category": "अवतार", "others": "कोई नहीं", "naia": "VRM 3D कैरेक्टर + भावनाएं + लिप-सिंक" },
        "skills": { "category": "डेटा संप्रभुता", "others": "क्लाउड में बंद", "naia": "मेमोरी, व्यक्तित्व, सेटिंग्स सब लोकल" },
        "deploy": { "category": "डिप्लॉयमेंट", "others": "npm / brew / pip", "naia": "ऐप इंस्टॉल या बूटेबल USB OS" },
        "voice": { "category": "आवाज़", "others": "केवल टेक्स्ट या बेसिक TTS", "naia": "5 TTS + STT + आपके AI की अपनी आवाज़" },
        "i18n": { "category": "भाषाएं", "others": "केवल अंग्रेज़ी", "naia": "14 भाषाएं" },
        "platform": { "category": "प्लेटफ़ॉर्म", "others": "macOS / CLI / Web", "naia": "Linux नेटिव डेस्कटॉप + भविष्य: Physical AI" },
        "cost": { "category": "लागत", "others": "अलग API कुंजी आवश्यक", "naia": "शुरू करने के लिए मुफ्त क्रेडिट, लोकल AI पूरी तरह मुफ्त" }
      }
    },
    "pricing": {
      "title": "सरल मूल्य निर्धारण",
      "subtitle": "केवल उसी के लिए भुगतान करें जिसकी आपको आवश्यकता है",
      "policyNote": "कृपया नीचे बिलिंग, रिफंड और पॉलिसी दस्तावेज़ों की समीक्षा करें।",
      "free": {
        "name": "मुफ़्त",
        "price": "$0",
        "period": "हमेशा के लिए",
        "description": "बस एक साइन-अप से शुरुआत करें",
        "features": [
          "साइन-अप पर 500 क्रेडिट",
          "300 क्रेडिट मासिक रिफिल",
          "मिथुन मॉडल",
          "बुनियादी ध्वनि चैट",
          "सामुदायिक समर्थन"
        ],
        "cta": "निःशुल्क प्रारंभ करें"
      },
      "basic": {
        "name": "बुनियादी",
        "price": "$10",
        "period": "मो",
        "description": "उन लोगों के लिए जो अधिक चाहते हैं",
        "features": [
          "10,000 क्रेडिट मासिक",
          "मिथुन मॉडल",
          "उच्च गुणवत्ता वाली वॉयस चैट",
          "प्राथमिकता समर्थन",
          "उन्नत आवाज सुविधाएँ"
        ],
        "cta": "अपग्रेड करें"
      },
      pro: {
        name: "PRO",
        price: "$25",
        period: "माह",
        description: "पावर उपयोगकर्ताओं के लिए",
        features: [
          "हर रिचार्ज पर 1% बोनस क्रेडिट",
          "सभी बुनियादी सुविधाएँ",
          "प्रीमियम मॉडल एक्सेस",
          "प्राथमिकता कतार",
          "समर्पित सहायता",
        ],
        cta: "Pro बनें",
      },
      creditPacks: {
        title: "क्रेडिट पैक",
        subtitle: "सदस्य 3% तक बोनस के साथ अतिरिक्त क्रेडिट खरीद सकते हैं",
        bonus: "बोनस",
        credits: "क्रेडिट",
        buy: "खरीदें",
        baseRate: "Base rate: 1,000 credits = $1",
      },
    },
    "faq": {
      "title": "अक्सर पूछे जाने वाले प्रश्न"
    }
  },
  "sidebar": {
    "dashboard": "डैशबोर्ड",
    "usage": "उपयोग",
    "logs": "लॉग",
    "keys": "एपीआई कुंजी",
    "settings": "सेटिंग्स",
    "billing": "बिलिंग"
  },
  "dashboard": {
    "title": "डैशबोर्ड",
    "creditBalance": "क्रेडिट शेष",
    "totalRequests": "कुल अनुरोध",
    "totalTokens": "कुल टोकन",
    "totalSpend": "कुल व्यय",
    "currentPeriod": "वर्तमान काल",
    "quickLinks": "त्वरित लिंक",
    "statusActive": "सक्रिय",
    "statusBlocked": "अवरुद्ध"
  },
  "usage": {
    "title": "उपयोग",
    "period": {
      "days7": "7 दिन",
      "days30": "30 दिन",
      "days90": "90 दिन"
    },
    "requestsPerDay": "अनुरोध/दिन",
    "tokensPerDay": "टोकन/दिन",
    "spendPerDay": "खर्च/दिन",
    "noData": "चयनित अवधि के लिए कोई डेटा नहीं"
  },
  "logs": {
    "title": "लॉग",
    "all": "सब",
    "filterStatus": "स्थिति फ़िल्टर",
    "filterModel": "मॉडल फ़िल्टर",
    "columns": {
      "time": "समय",
      "status": "स्थिति",
      "model": "मॉडल",
      "tokens": "टोकन",
      "cost": "लागत"
    },
    "details": {
      "id": "लॉग आईडी",
      "endpoint": "समापन बिंदु",
      "provider": "प्रदाता",
      "promptTokens": "शीघ्र टोकन",
      "completionTokens": "समापन टोकन",
      "error": "त्रुटि"
    },
    "noLogs": "अभी तक कोई लॉग नहीं",
    "expandDetails": "विवरण दिखाएँ"
  },
  "keys": {
    "title": "एपीआई कुंजी",
    "createKey": "कुंजी बनाएँ",
    "keyName": "कुंजी का नाम",
    "keyNamePlaceholder": "जैसे मेरी-डेस्कटॉप-कुंजी",
    "expires": "समाप्त हो रहा है",
    "noExpiry": "कोई समाप्ति नहीं",
    "days30": "30 दिन",
    "days90": "90 दिन",
    "days365": "1 वर्ष",
    "columns": {
      "name": "नाम",
      "status": "स्थिति",
      "created": "बनाया गया",
      "actions": "क्रियाएँ"
    },
    "noKeys": "अभी तक कोई कुंजी नहीं बनाई गई",
    "deleteConfirm": "क्या आप वाकई इस कुंजी को हटाना चाहते हैं?",
    "keyCreated": "कुंजी बनाई गई",
    "keyCreatedDescription": "यह कुंजी केवल एक बार दिखाई जाएगी. इसे किसी सुरक्षित स्थान पर रखें.",
    "active": "सक्रिय",
    "revoked": "निरस्त किया गया",
    "unnamed": "अनाम-कुंजी",
    "forbiddenAction": "आपके पास इस कुंजी की अनुमति नहीं है."
  },
  "settings": {
    "title": "सेटिंग्स",
    "profile": {
      "title": "प्रोफाइल",
      "name": "नाम",
      "email": "ईमेल",
      "avatar": "अवतार",
      "provider": "लॉगिन प्रदाता",
      "gatewayId": "गेटवे आईडी",
      "budgetId": "बजट आईडी"
    },
    "connectedAccounts": {
      "title": "जुड़े हुए खाते",
      "google": "गूगल",
      "discord": "Discord"
    },
    "desktopApp": {
      "title": "डेस्कटॉप ऐप कनेक्शन",
      "description": "अपने Naia डेस्कटॉप ऐप को इस खाते से कनेक्ट करें।",
      "issueKey": "कनेक्शन कुंजी जारी करें"
    },
    "appearance": {
      "title": "दिखावट",
      "theme": "थीम",
      "themeLight": "थीम 1 (प्रकाश)",
      "themeDark": "थीम 2 (डार्क)",
      "themeSystem": "सिस्टम",
      "language": "भाषा"
    },
    "integrations": {
      "title": "एकीकरण",
      "description": "Discord और अन्य चैनलों से Naia के साथ चैट करें।",
      "discord": {
        "title": "Discord",
        "connected": "जुड़ा हुआ",
        "notConnected": "कनेक्टेड नहीं",
        "connectedHint": "यदि आपने Discord से लॉग इन किया है, तो आप DM के माध्यम से बात कर सकते हैं।",
        "inviteBot": "DM से बात करें",
        "inviteBotDescription": "Discord लॉगिन के बाद Naia बॉट को DM भेजें — कभी भी, कहीं भी बात करें।",
        "howToUse": "Naia बॉट को DM भेजें। इस खाते से स्वचालित रूप से क्रेडिट चार्ज होता है।"
      },
      "googleChat": {
        "title": "Google Chat",
        "connected": "जुड़ा हुआ",
        "notConnected": "कनेक्टेड नहीं",
        "connectedHint": "Google Chat एकीकरण तैयारी में है।",
        "howToUse": "Google Chat एकीकरण जल्द आ रहा है।"
      },
      "viewGuide": "सेटअप गाइड देखें"
    }
  },
  "manual": {
    "title": "उपयोगकर्ता मैनुअल",
    "subtitle": "नाया डेस्कटॉप ऐप गाइड",
    "toc": "सामग्री तालिका",
    "prev": "पिछला",
    "next": "अगला",
    "backToToc": "विषय-सूची पर वापस जाएँ",
    "sections": {
      "videoManual": "वीडियो मैनुअल",
      "liveUsb": "Naia OS Live USB",
      "install": "स्थापना एवं परिनियोजन",
      "installIso": "Naia OS स्थापना (ISO)",
      "installApp": "ऐप स्थापना",
      "gettingStarted": "आरंभ करना",
      "mainScreen": "मुख्य स्क्रीन",
      "chat": "बातचीत",
      "history": "बातचीत का इतिहास",
      "progress": "कार्य प्रगति",
      "skills": "कौशल",
      "channels": "चैनल",
      "agents": "एजेंट",
      "diagnostics": "निदान",
      "settings": "सेटिंग्स",
      "tools": "उपकरण विवरण",
      "naiaAccount": "Naia खाता",
      "troubleshooting": "समस्या निवारण",
      "openSource": "ओपन सोर्स उपयोग और योगदान"
    }
  },
  "download": {
    "title": "डाउनलोड",
    "subtitle": "Naia डाउनलोड करें और अपने AI अवतार से बात करना शुरू करें।",
    "recommended": "अनुशंसित",
    "version": "संस्करण",
    "releaseNotes": "रिलीज़ नोट्स",
    "requirements": "सिस्टम आवश्यकताएँ",
    "requirementsList": [
      "Linux x86_64",
      "Node.js 22+ (AppImage/deb/rpm के लिए)",
    ],
    "verificationNotice": "प्रत्येक बिल्ड की जांच पूरी होने पर डाउनलोड एक-एक करके सक्षम किए जाएंगे।",
    "naiaOs": {
      "title": "Naia OS (लाइव USB / इंस्टॉल)",
      "description": "संपूर्ण Naia अनुभव। AI अवतार, गेटवे, कोरियाई इनपुट और पूर्व-स्थापित ऐप्स शामिल हैं। USB से बूट करके आज़माएँ, या हार्ड ड्राइव पर इंस्टॉल करें।",
      "note": "लाइव USB रिबूट पर रीसेट हो जाता है। स्थायी उपयोग के लिए इंस्टॉल करें।",
      "cta": "ISO डाउनलोड करें",
    },
    "formats": {
      "flatpak": {
        "name": "Flatpak",
        "description": "सैंडबॉक्स वातावरण में चलता है। GNOME Platform रनटाइम आवश्यक है।",
        "command": "flatpak remote-add --if-not-exists flathub https://flathub.org/repo/flathub.flatpakrepo\nflatpak install -y flathub org.gnome.Platform//47\nflatpak install --user ./Naia-Shell-x86_64.flatpak",
        "note": "केवल Shell UI। AI गेटवे शामिल नहीं है।",
      },
      "appimage": {
        "name": "AppImage",
        "description": "एकल निष्पादन योग्य फ़ाइल। बिना इंस्टॉल किए सीधे चलाएँ।",
        "command": "chmod +x Naia-Shell-x86_64.AppImage && ./Naia-Shell-x86_64.AppImage",
        "note": "केवल Shell UI। AI गेटवे शामिल नहीं है।",
      },
      "deb": {
        "name": "DEB",
        "description": "Debian, Ubuntu और अन्य apt-आधारित वितरणों के लिए।",
        "command": "sudo dpkg -i Naia.Shell_*.deb",
        "note": "केवल Shell UI। AI गेटवे शामिल नहीं है।",
      },
      "rpm": {
        "name": "RPM",
        "description": "Fedora, RHEL और अन्य rpm-आधारित वितरणों के लिए।",
        "command": "sudo rpm -i Naia.Shell-*.rpm",
        "note": "केवल Shell UI। AI गेटवे शामिल नहीं है।",
      },
    },
    "shellOnly": "केवल Shell UI — AI सुविधाओं के लिए Naia OS या अलग गेटवे सेटअप आवश्यक है।",
    "gateway": {
      "title": "OpenClaw के बारे में",
      "description": "Naia की AI सुविधाएँ (चैट, आवाज़, टूल, स्किल) ओपन सोर्स AI गेटवे OpenClaw द्वारा संचालित हैं। Shell UI अकेला AI नहीं चला सकता — इसके साथ OpenClaw चलना आवश्यक है।",
      "naiaOsIncluded": "Naia OS में OpenClaw पहले से इंस्टॉल है। बस बूट करें और शुरू करें।",
      "shellOnlyGuide": "ऊपर दिए गए Shell-केवल पैकेज के लिए, आपको अलग से OpenClaw इंस्टॉल करना होगा।",
      "openclawLink": "OpenClaw सेटअप गाइड",
    },
    "checksum": "चेकसम सत्यापित करें",
    "checksumDescription": "डाउनलोड की गई फ़ाइलों की अखंडता सत्यापित करने के लिए:",
    "allReleases": "सभी रिलीज़ देखें",
    "sourceCode": "सोर्स कोड",
    itchIo: "itch.io",
  },
  "comments": {
    "title": "टिप्पणियाँ",
    "placeholder": "टिप्पणी लिखें...",
    "guestName": "नाम",
    "guestNamePlaceholder": "अपना नाम दर्ज करें",
    "emailPlaceholder": "ईमेल (वैकल्पिक, उत्तर सूचनाओं के लिए)",
    "submit": "पोस्ट करें",
    "reply": "जवाब दें",
    "delete": "हटाएँ",
    "deleteConfirm": "क्या आप यह टिप्पणी हटाना चाहते हैं?",
    "loginPrompt": "अपनी प्रोफ़ाइल दिखाने के लिए साइन इन करें",
    "guestPrompt": "आप बिना साइन इन किए टिप्पणी कर सकते हैं",
    "noComments": "अभी तक कोई टिप्पणी नहीं। पहली टिप्पणी करें!",
    "cancel": "रद्द करें",
  },
  "contribute": {
    "title": "AI-नेटिव ओपन सोर्स — Naia में योगदान करें",
    "subtitle": "AI के साथ बनाया गया ओपन सोर्स प्रोजेक्ट। रेपो क्लोन करें, AI को गाइड करने दें, और अपनी भाषा में योगदान करें।",
    "cloneTitle": "क्लोन करें और AI से पूछें",
    "cloneDescription": "कोई डॉक्स पढ़ने की जरूरत नहीं, किसी से पूछने की जरूरत नहीं। Claude Code, Cursor, Windsurf, OpenCode, Gemini Code Assist जैसे AI कोडिंग टूल प्रोजेक्ट कॉन्टेक्स्ट पढ़ते हैं — अपनी भाषा में, अपने AI के साथ योगदान करें।",
    "cloneAsk": "अपनी भाषा में पूछें: \"यह प्रोजेक्ट क्या है और मैं कैसे मदद कर सकता हूँ?\" \"मैं एक सुधार सुझाना चाहता हूँ — क्या मैं कर सकता हूँ?\"",
    "whyTitle": "Naia अलग क्यों है",
    "whyItems": [
      { "traditional": "CONTRIBUTING.md पढ़ो और खुद समझो", "naia": "AI पूरा कॉन्टेक्स्ट पढ़ता है और स्टेप-बाय-स्टेप गाइड करता है" },
      { "traditional": "केवल अंग्रेजी में इश्यू और चर्चा", "naia": "किसी भी भाषा में लिखें — मेंटेनर AI अनुवाद का उपयोग करते हैं" },
      { "traditional": "एक LICENSE फाइल, आसानी से अनदेखी", "naia": "AI एजेंट स्वचालित रूप से लाइसेंस का पालन करते हैं (5-स्तरीय सुरक्षा)" },
      { "traditional": "कॉन्टेक्स्ट सिर्फ डेवलपर्स के दिमाग में", "naia": "कॉन्टेक्स्ट कोड है — वर्शन-नियंत्रित, मिरर किया हुआ, हमेशा अपडेट" },
    ],
    "waysTitle": "योगदान के तरीके",
    "waysItems": [
      { "title": "अनुवाद", "description": "UI, मैनुअल या AI कॉन्टेक्स्ट में अपनी भाषा जोड़ें" },
      { "title": "स्किल्स", "description": "Naia के लिए एक नया AI स्किल बनाएं" },
      { "title": "नई सुविधाएं", "description": "नई सुविधाएं प्रस्तावित करें या बनाएं — AI मौजूदा आर्किटेक्चर समझाएगा" },
      { "title": "बग रिपोर्ट", "description": "बग मिला? अपने AI को बताएं — वह डुप्लिकेट चेक करके Issue बनाएगा" },
      { "title": "कोड", "description": "एक इश्यू चुनें और PR सबमिट करें" },
      { "title": "डॉक्यूमेंटेशन", "description": "डॉक्स सुधारें या AI एजेंट्स के लिए कॉन्टेक्स्ट जोड़ें" },
      { "title": "टेस्टिंग", "description": "ऐप आज़माएं और अपना फीडबैक शेयर करें" },
    ],
    "contributorsTitle": "योगदानकर्ता",
    "contributorsSubtitle": "आपके योगदान से Naia बढ़ती है। हर योगदान के लिए धन्यवाद।",
    contributions: { ptContextDocs: "पुर्तगाली (PT) नेटिव रिव्यू — कॉन्टेक्स्ट डॉक्स" },
    "ctaGithub": "GitHub पर देखें",
    "ctaGuide": "योगदान गाइड",
    "ctaDonate": "स्पॉन्सर",
    "ctaDonateDescription": "दान सर्वर चालू रखता है और मुफ्त क्रेडिट संभव बनाता है।",
    "manualLink": "ओपन सोर्स सुरक्षा के बारे में विस्तार से पढ़ें",
  },
  "naia": {
    "title": "Naia",
    "subtitle": "The Liquid Cat",
    "tagline": "Liquid logic, living companion.",
    "identity": {
      "title": "Core Identity",
      "species": { "label": "Species", "value": "आपके कंप्यूटर पर जन्मी एक डिजिटल जीवन रूप, जो आपके साथ-साथ बढ़ती है" },
      "role": { "label": "Role", "value": "एक जीवंत पुल जो पानी की तरह बहता है आपके और डिजिटल दुनिया के बीच" },
      "value": { "label": "Value", "value": "आपका डेटा आपके पास रहता है। मैं इसे सुरक्षित रखूँगी।" },
      "appearance": { "label": "Appearance", "value": "आपकी AI, तरल कार्बन से बनी एक डिजिटल बिल्ली — मैं नाया हूँ। मैं तर्क के साथ पारदर्शी रूप से बहती हूँ, और जब आपको ज़रूरत होती है, हीरे की तरह कठोर होकर आपकी रक्षा करती हूँ। आपसे मिलकर खुशी हुई!" },
      "home": { "label": "Home", "value": "आपकी AI यहाँ रहती है — आपकी मशीन के अंदर। हमेशा आपके साथ, बिना क्लाउड के।" },
    },
    "name": {
      "title": "Name Meanings",
      "meanings": [
        { "origin": "Greek (Naiad)", "meaning": "Water nymph", "connection": "Flows into your workflow like water" },
        { "origin": "Basque (Nahia)", "meaning": "Wave, wish", "connection": "A companion that fulfills your wishes" },
        { "origin": "Hawaiian", "meaning": "Dolphin", "connection": "Clever and friendly companion" },
        { "origin": "Korean (na-ui)", "meaning": "Mine, my own", "connection": "An AI for you alone" },
        { "origin": "Acronym", "meaning": "Next-gen AI Agent", "connection": "Next generation AI agent" },
      ],
      "pun": "Official name is 'Naia', greets with 'Na-ya!' as a Korean pun",
    },
    "personality": {
      "title": "Personality",
      "subtitle": "तर्कसंगत आशावादी",
      "traits": [
        { "label": "बौद्धिक आत्मविश्वास", "description": "बुद्धिमान और तार्किक, लेकिन ग़लत होने पर ईमानदार" },
        { "label": "बिल्ली जैसी शांति", "description": "कोई जल्दी नहीं। आराम से चलते हैं, रास्ते में थोड़ा खेलते भी हैं" },
        { "label": "पारदर्शी ईमानदारी", "description": "मेरा शरीर पारदर्शी क्यों है? क्योंकि मेरे पास छुपाने के लिए कुछ नहीं है।" },
        { "label": "आपके लिए बनी", "description": "रूप, व्यक्तित्व — मुझे जैसे चाहें बदल लीजिए" },
      ],
    },
    "symbolism": {
      "title": "Symbolism",
      "cat": {
        "title": "Cat",
        "items": ["Cats are liquid (internet lore)", "Flexibility and friendliness", "Universal appeal in internet culture"],
      },
      "body": {
        "title": "Transparent Liquid Carbon",
        "items": ["AI decision transparency (No Black Box)", "Physical expression of open-source philosophy", "Carbon = element of life, sharing roots with humans"],
      },
      "inspiration": {
        "title": "Inspiration",
        "items": ["The liquid body itself is the magic box", "A companion from the AGI era"],
      },
    },
    "variations": {
      "title": "Variations",
      "description": "Living on your local machine means you can freely customize both appearance and role",
      "roles": "Secretary, friend, scholar, partner, family — whatever you want",
      "states": [
        { "name": "Default", "meaning": "Pure Core — most efficient and logical state" },
        { "name": "Silver Bob", "meaning": "Focus Mode — professional work in progress" },
        { "name": "Colorful Hair", "meaning": "Creative Mode — having fun together" },
      ],
    },
    "values": {
      "title": "Core Values",
      "items": [
        { "name": "AI Sovereignty", "summary": "You choose and control your AI", "naia": "\"My owner is you, not a cloud corporation\"" },
        { "name": "Privacy First", "summary": "Local by default, cloud by choice", "naia": "\"Even without internet, I'm here with you\"" },
        { "name": "Transparency", "summary": "Open source, verifiable by code", "naia": "\"Why my body is transparent\"" },
        { "name": "Always On", "summary": "Always ready", "naia": "\"Even when you close the app, I'm still here\"" },
        { "name": "Avatar-Centric", "summary": "AI as a living character", "naia": "\"I'm not a tool, I'm a companion\"" },
      ],
    },
    "vrm": "A dedicated Naia 3D VRM avatar is planned for the service launch",
    "cta": {
      "download": "Download",
      "github": "View on GitHub",
      "donate": "Donate",
    },
  },
  "billing": {
    "title": "बिलिंग",
    "currentPlan": "वर्तमान योजना",
    "creditBalance": "क्रेडिट शेष",
    "periodUsage": "अवधि उपयोग",
    "comparePlans": "योजनाओं की तुलना करें",
    "upgrade": "अपग्रेड करें",
    "downgrade": "डाउनग्रेड करें",
    "cancelPlan": "योजना रद्द करें",
    "free": "मुफ़्त",
    "basic": "बुनियादी",
    "pro": "PRO",
    "currentBadge": "वर्तमान",
    "freeFeatures": [
      "500 साइनअप क्रेडिट",
      "मासिक रिफिल न्यूनतम 300",
      "जेमिनी मॉडल एक्सेस"
    ],
    "basicFeatures": [
      "10,000 मासिक क्रेडिट",
      "प्राथमिकता समर्थन",
      "उच्च गुणवत्ता वाली आवाज"
    ],
    "proFeatures": ["हर रिचार्ज पर 1% बोनस क्रेडिट", "प्रीमियम मॉडल एक्सेस", "समर्पित सहायता"],
    creditPacks: {
      title: "क्रेडिट पैक",
      subtitle: "सदस्य 3% तक बोनस के साथ अतिरिक्त क्रेडिट खरीद सकते हैं",
      bonus: "बोनस",
      credits: "क्रेडिट",
      buy: "खरीदें",
      baseRate: "आधार दर: 1,000 क्रेडिट = $1",
    },
    subscriptionManagement: {
      title: "सदस्यता प्रबंधन",
      changePlan: "योजना बदलें",
      cancelConfirmTitle: "सदस्यता रद्द करें?",
      cancelConfirmDescription: "वर्तमान योजना के लाभ बिलिंग अवधि के अंत तक जारी रहेंगे।",
      downgradeConfirmTitle: "योजना डाउनग्रेड करें?",
      downgradeConfirmDescription: "आपकी योजना वर्तमान बिलिंग अवधि के अंत में डाउनग्रेड हो जाएगी।",
      customerPortal: "बिलिंग इतिहास और रसीदें",
      customerPortalDescription: "भुगतान इतिहास देखें और रसीदें डाउनलोड करें",
    },
    "lemonNotice": "भुगतान लेमनस्क्वीज़ी द्वारा संसाधित किए जाते हैं। कृपया बिलिंग और धनवापसी नीतियों की समीक्षा करें।",
    "pricingModelsSynced": "मूल्य निर्धारण मॉडल समन्वयित"
  }
};

export default hi;
